...
!FKB module to load and inference network
...
!For storing ML statics (Mean and standard deviation)
type ml_stats
   !Mean and standard deviation for various inputs and outputs
   real(r8) :: mean, std_dev
end type ml_stats

!define stat variables for temperature [k]; pressure[hPa]; vertical velocoty[cm/s],
!and relative humidity[unitless]
type(ml_stats) :: temp_stats, pmid_stats, updraft_stats, relhum_stats
!define stats variables for hygroscopicity(kappa)[unitless], aerosol numbers[#/cm3], particle radius[micron],
!activated fractions[unitless] form the ml routine
type(ml_stats), dimension(4) :: kappa_stats, num_stats, radius_stats
...
logical :: lq(pcnst) = .false. ! set flags true for constituents with non-zero tendencies
                               ! in the ptend object
!initiate network type instances
type(network_type) :: activated_fraction_net

logical, parameter :: call_default_activation = .false.
...
subroutine ndrop_init
   ...
   !Initialize statistics needed for Machine Learning emulator:
   call ml_stats_init()
...
subroutine ml_stats_init()

  implicit none

  !----------------------------------------------------------------
  !In this subroutine, we initialize all the statics for
  !variables which are need by the ML routines
  !----------------------------------------------------------------

  !------------------------------------------------------------------------------------
  !Stats (mean and std. dev) for input from ML call
  !------------------------------------------------------------------------------------
  !temprature [k]
  temp_stats%mean    = 276.5568_r8
  temp_stats%std_dev = 19.8578071594238_r8

  !Pressure[hPa]
  pmid_stats%mean    = 864.0167_r8
  pmid_stats%std_dev = 142.514663696289_r8

  !Kappa  (hygroscopicity)[unitless]
  !-- 1st mode
  kappa_stats(1)%mean    = 0.5166725_r8
  kappa_stats(1)%std_dev = 0.223736_r8
  !-- 2nd mode
  kappa_stats(2)%mean    = 0.4451128_r8
  kappa_stats(2)%std_dev = 0.1278386_r8
  !-- 3rd mode
  kappa_stats(3)%mean    = 0.87421_r8
  kappa_stats(3)%std_dev = 0.3372654_r8
  !-- 4th mode
  kappa_stats(4)%mean    = 3.335759e-06_r8
  kappa_stats(4)%std_dev = 1.104734e-05_r8

  !aerosol number  [#/cm3]
  !-- 1st mode
  num_stats(1)%mean     = 142.9523_r8
  num_stats(1)%std_dev  = 225.3325_r8
  !-- 2nd mode
  num_stats(2)%mean     = 315.676_r8
  num_stats(2)%std_dev  = 742.0457_r8
  !-- 3rd mode
  num_stats(3)%mean     = 0.5773571_r8
  num_stats(3)%std_dev  = 0.9817318_r8
  !-- 4th mode
  num_stats(4)%mean     = 60.9462_r8
  num_stats(4)%std_dev  = 432.6927_r8

  !particle radius in microns [micron]
  !-- 1st mode
  radius_stats(1)%mean       = 0.07111134_r8
  radius_stats(1)%std_dev    = 0.01116247_r8
  !-- 2nd mode
  radius_stats(2)%mean       = 0.01859621_r8
  radius_stats(2)%std_dev    = 0.003865036_r8
  !-- 3rd mode
  radius_stats(3)%mean       = 0.6128845_r8
  radius_stats(3)%std_dev    = 0.08603426_r8
  !-- 4th mode
  radius_stats(4)%mean       = 0.05734761_r8
  radius_stats(4)%std_dev    = 0.007762632_r8

  !updraft velocity
  updraft_stats%mean         = 14.03403_r8
  updraft_stats%std_dev      = 14.3349313735962_r8

  !relative humidity
  relhum_stats%mean          = 98.77437_r8
  relhum_stats%std_dev       = 16.2376289367676_r8

end subroutine ml_stats_init
...
subroutine dropmixnuc( &
   ...
   integer  :: loop_up_bnd         
   real(r8), pointer :: qv(:,:)      ! water vapor mixing ratio (kg/kg-of-air)
   ...
   real(r8), pointer :: dgncur(:,:,:)  !dry diameter of a particle (m) [dimensions:(ncol,pver,nmodes)]
...
!+++ AeroCOM IND3 output
   ...
   qv       => state%q(:,:,1) !water vapor mixing ratio(kg/kg-of-air)
   ...
   call pbuf_get_field(pbuf, dgnum_idx, dgncur)
   ...
   do i = 1, ncol
      ...
      do k = top_lev, pver
         ...
         if (cldn_tmp-cldo_tmp > 0.01_r8) then
            ...
            if(call_default_activation) then
            call activate_modal( &
               ...
               !Compute activated faction (fn), mass (fm) and fluxes (fluxn, fluxm)
               !Following subroutine call involkes ML emulator to compute activated
               !fraction(fn) and  derive mass fraction (fm) and fluxes(fluxn, fluxm)
               !based on new activated fraction(fn)
               call ml_activate_modal(i, k, lchnk,             & !unused input (for debugging only)
                    ntot_amode, qv(i,k), temp(i,k), pmid(i,k), & !input
                    hygro, naermod, wbar, dgncur(i,k,:),       & !input
                    fn,                                        & !input-output
                    fm, fluxn, fluxm, flux_fullact(k))           !output
            endif
         ...
         if (cldn(i,k) > 0.01_r8) then
            ...
            if (cldn(i,k) - cldn(i,kp1) > 0.01_r8 .or. k == pver) then
               ...
               call activate_modal( &
                  ...
                  !Compute activated faction (fn), mass (fm) and fluxes (fluxn, fluxm)
                  !Following subroutine call involkes ML emulator to compute activated
                  !fraction(fn) and derives mass fraction (fm) and fluxes(fluxn, fluxm)
                  !based on new activated fraction(fn)
                  call ml_activate_modal(i, k, lchnk,             & !unused input (for debugging only)
                       ntot_amode, qv(i,k), temp(i,k), pmid(i,k), & !input
                       hygro, naermod, wbar, dgncur(i,k,:),       & !input
...
!===============================================================================
subroutine ml_activate_modal(icol, klev, lchnk,        & !unused input (for debugging only)
     nmode, qv, temp, pmid,                            & !input
     hygro, naermod, wbar, dgncur,                     & !input
     fn_act_modal,                                     & !output
     fmass, fluxn, fluxm, flux_fullact)                  !output

  !-------------------------------------------------------------------------------------------------
  !Purpose: A subroutine to  compute activated fraction of aerosol number using machine learning (ML)
  !algorithm. Activated mass fraction, fluxes for number and mass are then derived from
  !activated number fraction using equations 13 and 14 from "A parameterization of aerosol
  !activation 2. Multiple aerosol types by Abdul-Razzak and Ghan
  !
  !Method: 1. Normalize inputs
  !        2. call ML functions to compute activated fraction
  !        4. Derive activated mass fractions and fluxes
  !
  !Algorithm: Sungduk Yu (UCI)  provided the trained machine learning algorithms
  !
  !Author:  Balwinder Singh (with inputs from Po-Lun Ma and Mike Pritchard)
  !
  !calls: activated_fraction_net%output (ML library function),
  !       normalize functions
  !-------------------------------------------------------------------------------------------------

  !--------------------------------------------------------------------------------------------
  !Future work:
  !. Handle precision from rk to r8 consistently, rk is lower precision
  !. How to handle cases when number concentration is zero or less?
  !. Units of fluxes (flumnum, fluxmass, flux_fullact) are mentioned as cm/s above. In this
  !  subroutine, we are converting wbar to cm/s, so the units are consistent.In activate_modal
  !  suboroutine, these fluxes are obtained using wbar, which has units of m/s.
  !  I am not sure if that is okay or if we need to change something in this subroutine.
  !. Currently we limit activated number fraction output to 0.99 or less. Activate_modal
  !  routine ends the simulation if activated fraction is > 1.01.
  !--------------------------------------------------------------------------------------------

  use mod_kinds,       only: rk          !ML model precision
  use shr_log_mod ,    only: errMsg => shr_log_errMsg !For writing error messages
  use time_manager,    only: get_nstep   !To get current time step for error messages

  implicit none
  !inputs
  integer,  intent(in) :: icol, klev, lchnk   !indices for debugging only
  integer,  intent(in) :: nmode               !number of modal aerosol modes
  real(r8), intent(in) :: qv                  !water vapormixing ratio (kg/kg of air)
  real(r8), intent(in) :: temp                !temperature (K)
  real(r8), intent(in) :: pmid                !pressure (Pa)
  real(r8), intent(in) :: hygro(nmode)        !bulk hygroscopicity of a mode (Kappa; unitless)
  real(r8), intent(in) :: naermod(nmode)      !number concentration (1/m3)
  real(r8), intent(in) :: wbar                !updraft velocity (m/s)
  real(r8), intent(in) :: dgncur(nmode)       !dry diameter (m)

  !outputs
  real(r8), intent(out):: fn_act_modal(nmode) !activated number fraction(unitless)
  real(r8), intent(out):: fmass(nmode) !activated mass fraction (unitless)
  real(r8), intent(out):: fluxn(nmode) !activated aerosol number frac flux into cloud (cm/s)~balli-comment about units in email
  real(r8), intent(out):: fluxm(nmode) !activated aerosol mass frac flux into cloud (cm/s)
  real(r8), intent(inout):: flux_fullact !same units as wbar (m/s)
  !local
  integer, parameter :: INPUT_SIZE = 16 ! size of the input vector for ML emulator
  integer  :: imode

  real(r8), parameter :: tiny_real = epsilon(1.0_r8)
  real(r8) :: qv_sat, es_tmp, relhum !for computing relative humidity [unitless]
  real(r8) :: wbar_in_cm_per_sec, pmid_in_hpa
  real(r8) :: num_conc_in_cm3(nmode), radius_in_microm(nmode)

  !--For storing normalized values
  real(r8) :: num_conc(nmode)
  real(r8) :: temp_norm, pmid_norm, relhum_norm, up_norm
  real(r8) :: radius_norm(nmode), num_norm(nmode), kappa_norm(nmode)

  real(rk), allocatable :: fn_ml_out(:) !arrays to store ML output

  real(rk) :: input_ml(INPUT_SIZE)    !input vector for ML function call
  real(r8) :: erf_u(1), u_fn(1)  !size 1 arrays as vderfinv take vectors as input
  real(r8) :: arg
  ...
  !compute saturation specific humidity (qv_sat [kg/(kg of air)])
  ![NOTE: es_tmp (saturation vapor pressure) is not used anywhere in this subroutine]
  call qsat(temp, pmid, es_tmp, qv_sat)

  !compute relative humidity [unitless]
  relhum = max( 0.0_r8, min( 1.0_r8, qv/qv_sat ) )

  !===========================================================================
  ! Prepare input for ML library function call
  !. convert units
  !. normalize
  !===========================================================================

  !------------------------------------------------------------------------------------------
  !Convert units: (number concentration, radius, updraft velocity and pmid)
  !------------------------------------------------------------------------------------------

  !number concentration (convert from 1/m3 to 1/cm3 for each mode)
  num_conc_in_cm3(:)  = naermod(:) * 1.0e-6_r8

  !dry radius of particles (convert from m to micrometer)
  radius_in_microm(:) =  (dgncur(:)*0.5_r8) * 1.0e6_r8 !dgncur is diameter, convert it to radius and convert units

  !updraft velocity (m/s to cm/s)
  wbar_in_cm_per_sec = wbar * 100.0_r8

  !pressure (Pa to hPa)
  pmid_in_hpa = pmid * 0.01_r8

  !------------------------------------------------------------------------------------------
  !Normalize input (applicable to all modes)
  !------------------------------------------------------------------------------------------

  temp_norm    = normalize(temp,               temp_stats%mean,    temp_stats%std_dev   )
  pmid_norm    = normalize(pmid_in_hpa,        pmid_stats%mean,    pmid_stats%std_dev   )
  relhum_norm  = normalize(relhum,             relhum_stats%mean,  relhum_stats%std_dev )
  up_norm      = normalize(wbar_in_cm_per_sec, updraft_stats%mean, updraft_stats%std_dev)

  do imode = 1, nmode
     !Normalize input (as a function of imode)
     radius_norm(imode)       = normalize(radius_in_microm(imode), radius_stats(imode)%mean, radius_stats(imode)%std_dev)
     num_norm(imode)          = normalize(num_conc_in_cm3(imode),  num_stats(imode)%mean,    num_stats(imode)%std_dev   )
     kappa_norm(imode)        = normalize(hygro(imode),            kappa_stats(imode)%mean,  kappa_stats(imode)%std_dev )
  enddo


  !if it is a test run, initialize input variables with hardwired numbers for a test case
  if(do_test) then
     call populate_input_for_test(temp_norm, pmid_norm, relhum_norm, up_norm, num_norm(1), num_norm(2), &
       num_norm(3), num_norm(4), radius_norm(1), radius_norm(2), radius_norm(3), &
       radius_norm(4), kappa_norm(1), kappa_norm(2), kappa_norm(3), &
       kappa_norm(4))
  endif

  !Store input in a vector (required by ML routine)
  !Input variables should be in the following order:
  ![tair, pressure, rh, wbar,
  !num_aer(1), num_aer(2), num_aer(3), num_aer(4),
  !r_aer(1), r_aer(2), r_aer(3), r_aer(4),
  !kappa(1), kappa(2), kappa(3), kappa(4)]
  input_ml = real([temp_norm, pmid_norm, relhum_norm, up_norm, &
       num_norm(1), num_norm(2), num_norm(3), num_norm(4), &
       radius_norm(1), radius_norm(2), radius_norm(3), radius_norm(4), &
       kappa_norm(1), kappa_norm(2), kappa_norm(3), kappa_norm(4)],rk)

  !call ML function to compute activation fraction
  fn_ml_out   = activated_fraction_net%output(input_ml)

  !ensure that the output size is what we expect
  if(size(fn_ml_out) .ne. nmode)then
     write(err_str,*)'The output from the emulator should be of size ',nmode,' but the size ', &
          'is:',size(fn_ml_out),' at time step:',get_nstep(),errmsg(__FILE__,__LINE__)
     write(iulog,*)trim(err_str)
     call endrun(trim(err_str))
  endif

  if(do_test) then
     !since it is a test, we need to print out the results
     !Results will be printed in fort.101 file in the run directory
     if(masterproc) then ! only do it for masterproc otherwise the I/O will overwhelm simulation
        write(101,*)'Activated Fraction:'
        do imode = 1, nmode
           write(101,*)'fn:',imode, real(fn_ml_out(imode),r8)
        enddo
     endif
     !NOTE: Not calling endrun as sometime the run ends without writing out the results
     !we can use "flush" but printing only for the masterproc also works.
     write(err_str,*)'Endrun called as it is a test; set do_test to .false. to run a simulation', &
          ',time step is:',get_nstep(),errmsg(__FILE__,__LINE__)
     write(iulog,*)trim(err_str)
     !call endrun(trim(err_str))
  endif

  !Activated fraction
  fn_act_modal(:) = real(fn_ml_out(:),r8) ! convert to real type r8

  !deallocate (FKB allocates this during each call, so we need to deallocate this here)
  deallocate(fn_ml_out)

  !compute activated fraction, mass fraction, number flux and mass flux
  do imode = 1, nmode

     !limit activated fraction to be less than 0.99
     fn_act_modal(imode) = min(abs(fn_act_modal(imode)), 0.99_r8) !do we need abs?

     !limit activated fraction to be slightly above the machine precision if it goes below
     !as it becomes zero below the machine precision.
     !A zero activated fraction will make erf(1)=1.0, which will yeild
     !infinity for u_fn(1) when obtained via call to vderfinv
     if (fn_act_modal(imode)<tiny_real)fn_act_modal(imode)= tiny_real*10.0_r8

     !------------------------------------------------------------------------------------------------
     !compute "u" for the error function from given activated number fraction (assuming single draft)
     !(equation 13 from "A parameterization of aerosol activation 2. Multiple aerosol types
     !by Abdul-Razzak and Ghan)
     !------------------------------------------------------------------------------------------------
     erf_u(1) = 1._r8 - (fn_act_modal(imode)/0.5_r8)

     !call inverse of error function to obtain "u"
     !This function only works on vectors (not single value variables)
     call vderfinv(1, erf_u(1), u_fn(1))

     !------------------------------------------------------------------------------------------------
     !compute activated mass fraction
     !(equation 14 from "A parameterization of aerosol activation 2. Multiple aerosol types
     !by Abdul-Razzak and Ghan)
     !------------------------------------------------------------------------------------------------
     arg = u_fn(1)-1.5_r8*sq2*alogsig(imode)
     fmass(imode) = 0.5_r8*(1._r8-erf(arg))

     !Compute fluxes
     fluxn(imode) = fn_act_modal(imode) * wbar_in_cm_per_sec ![cm/s]!balli--- check if we need to convert units to m/s or not
     fluxm(imode) = fmass(imode)        * wbar_in_cm_per_sec ![cm/s]!balli--- check if we need to convert units to m/s or not
  enddo

  return

end subroutine ml_activate_modal

pure function normalize(invar, mean, stddev) result(norm)
  !inputs
  real(r8), intent(in) :: invar, mean, stddev

  !output
  real(r8) :: norm

  norm = (invar - mean)/stddev

  return

end function normalize

pure function denormalize(norm, mean, stddev) result(denorm)


  !inputs
  real(r8), intent(in) :: norm
  real(r8), intent(in) :: mean, stddev

  !output
  real(r8) :: denorm

  denorm = norm * stddev + mean

  return

end function denormalize

!===============================================================================

subroutine populate_input_for_test(temp_norm, pmid_norm, relhum_norm, up_norm, num_norm1, num_norm2, &
     num_norm3, num_norm4, radius_norm1, radius_norm2, radius_norm3, &
     radius_norm4, kappa_norm1, kappa_norm2, kappa_norm3, &
     kappa_norm4)

  implicit none

  real(r8), intent(out) ::temp_norm, pmid_norm, relhum_norm, up_norm, num_norm1, num_norm2, &
       num_norm3, num_norm4, radius_norm1, radius_norm2, radius_norm3, &
       radius_norm4, kappa_norm1, kappa_norm2, kappa_norm3, &
       kappa_norm4

  temp_norm    = normalize(288.1197_r8,  temp_stats%mean,            temp_stats%std_dev)
  pmid_norm    = normalize(893.8077_r8,  pmid_stats%mean,            pmid_stats%std_dev)
  relhum_norm  = normalize(99.40067_r8,  relhum_stats%mean,          relhum_stats%std_dev)
  up_norm      = normalize(15.85456_r8,  updraft_stats%mean,    updraft_stats%std_dev)
  num_norm1    = normalize(57.13362_r8,  num_stats(1)%mean,     num_stats(1)%std_dev)
  num_norm2    = normalize(192.5464_r8,  num_stats(2)%mean,     num_stats(2)%std_dev)
  num_norm3    = normalize(0.7979687_r8,  num_stats(3)%mean,     num_stats(3)%std_dev)
  num_norm4    = normalize(0.03749337_r8, num_stats(4)%mean,     num_stats(4)%std_dev)
  radius_norm1 = normalize(0.0782353_r8,  radius_stats(1)%mean,       radius_stats(1)%std_dev)
  radius_norm2 = normalize(0.01449036_r8,  radius_stats(2)%mean,       radius_stats(2)%std_dev)
  radius_norm3 = normalize(0.6682749_r8,  radius_stats(3)%mean,       radius_stats(3)%std_dev)
  radius_norm4 = normalize(0.04999997_r8,  radius_stats(4)%mean,       radius_stats(4)%std_dev)
  kappa_norm1  = normalize(0.8289536_r8, kappa_stats(1)%mean,   kappa_stats(1)%std_dev)
  kappa_norm2  = normalize(0.377653_r8, kappa_stats(2)%mean,   kappa_stats(2)%std_dev)
  kappa_norm3  = normalize(1.153206_r8,  kappa_stats(3)%mean,   kappa_stats(3)%std_dev)
  kappa_norm4  = normalize(1.46842e-06_r8, kappa_stats(4)%mean,   kappa_stats(4)%std_dev)

end subroutine populate_input_for_test
...
subroutine activate_modal(wbar, sigw, wdiab, wminf, wmaxf, tair, rhoair,  &
   ...
   real(r8) wmin,wmax,w_tmp,dw,dwmax,dwmin,wnuc,dwnew,wb
...
100      wnuc=w_tmp+wdiab
         ...
         sumflx_fullact = sumflx_fullact &
            + sixth*(wb*(g+gold) + (g*w_tmp+gold*wold))*dw
         !            sumg=sumg+0.5_r8*(g+gold)*dw
         gold=g
         wold=w_tmp
         dw=dwnew
         if(n.gt.1.and.(w_tmp.gt.wmax.or.fnmin.gt.fmax))go to 20
         w_tmp=w_tmp+dw
      enddo
      write(iulog,*)'do loop is too short in activate'
      write(iulog,*)'wmin=',wmin,' w=',w_tmp,' wmax=',wmax,' dw=',dw
...
20    continue
      ...
      do m=1,nmode
         ...
         if(fn(m).gt.1.01_r8)then
            ...
            write(iulog,*)'w,m,na,amcube=',w_tmp,m,na(m),amcube(m)
